...
redefineEventTargetPrototype(
  ...
  (
    ...
    if (CapturingEvents.has(type)) {
      if (!CapturedListeners.has(currentEventTarget))
        CapturedListeners.set(currentEventTarget, xray_Map())
...
export function dispatchEventRaw<T extends Event>(
  ...
  if (!CapturingEvents.has(type))
    return warn(
      "[soda-event-util/injected-script] Trying to send event didn't captured."
    )
    ...
    return new no_xray_Proxy(
      ...
      clone_into({
        get(target, key) {
          ...
          return (
            (source as any)[key] ??
            (unwrapXRay_CPPBindingObject(target) as any)[key]
          )
        }
      })
    )
