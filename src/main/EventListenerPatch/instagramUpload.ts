...
export async function instagramUpload(url: string) {
  ...
  const file = constructXrayUnwrappedFilesFromUintLike(
    'image/jpeg',
    'image.jpg',
  ...
  const postButton = document.querySelector<HTMLElement>(
    ...
    overwriteFunctionOnXRayObject(
      ...
      (_target: Function, thisArg, args) => {
        ...
        for (const x of Object.keys(raw)) {
          ...
          if (
            x.startsWith('__reactEventHandlers') ||
