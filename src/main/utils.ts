...
export function constructXrayUnwrappedDataTransferProxy(
  xrayUnwrappedFile: File
) {
...
export function sendEvent<T extends keyof InternalEvents>(
  event: T,
  ...args: InternalEvents[T]
) {
  const detail = encodeEvent(event, args)
  apply(dispatchEvent, document, [
    new no_xray_CustomEvent(CustomEventId, { detail })
  ])
}
